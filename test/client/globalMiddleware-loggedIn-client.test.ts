...
describe('Global middleware, logged in, client-side', async () => {
  ...
  it('allow:logged-in renders page', async () => {
    ...
    await page.waitForURL(`${useTestContext().url}global/allow/logged-in`, { timeout: 1000 })
  ...
  it('deny:logged-out renders page', async () => {
    ...
    await page.waitForURL(`${useTestContext().url}global/deny/logged-out`, { timeout: 1000 })
